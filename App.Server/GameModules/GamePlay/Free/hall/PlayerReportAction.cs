...
namespace App.Server.GameModules.GamePlay.Free.hall
{
    ...
    class PlayerReportAction : AbstractPlayerAction
    {
        ...
        private void SurvivalRank(List<IPlayerInfo> players, Dictionary<long, GameOverPlayer> goPlayerList)
        {
            if (players.Count < 1)
                return;
            ...
            foreach (IPlayerInfo ipi in players)
            {
                ...
                if (teamDeadDict.ContainsKey(ipi.TeamId))
                {
                    ...
                    {
                        teamDeadDict[ipi.TeamId] = ipi.StatisticsData.LastDeadTime;
                    }
                }
                else
                {
                    teamDeadDict.Add(ipi.TeamId, ipi.StatisticsData.LastDeadTime);
                }
            }
            //队伍总数
            ...
            foreach (IPlayerInfo ipi in players)
            {
                ipi.StatisticsData.Rank = ipi.StatisticsData.IsRunaway && ipi.StatisticsData.DeathOrder == 0 ? teamCount : teamRankDict[ipi.TeamId];
                GameOverPlayer gop;
